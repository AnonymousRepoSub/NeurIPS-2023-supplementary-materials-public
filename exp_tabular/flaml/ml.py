...
import logging
from .group import get_test_group
...
def metric_loss_score(
    ...
    if is_in_sklearn_metric_name_set(metric_name):
        ...
        try:
            ...
            # metric = evaluator(y_processed_true, y_processed_predict)
            # print('score_dict', score_dict, metric)
...
def get_val_loss(
    ...
    if require_train:
        ...
        # print(f'fit ends in {time.time()-start}, budget {budget}', flush=True)
    s = time.time()
    ...
    # print("Whole data loss", val_loss, list(y_val))
    # print("lenght of x_avl", len(X_val))
    # print(f'eval time: {time.time() - s}', flush=True)
    # print(groups_val, len(groups_val))
    # # print("---------------------month--------------------") # check
    tmp_dict = get_test_group(train_len=len(X_train)) # train_len is included in file name
    group_value = tmp_dict['group_value']
    group_num = tmp_dict['group_num']
    if group_num > 0:
        val_len = len(X_val)

        if group_value is None:
            fold_length = int(val_len / group_num)
            group = []
            for i in range(group_num):
                group += [i] * fold_length
                if i == group_num-1 and len(group) != val_len:
                    group += [group_num-1] * (val_len - len(group))
            group = np.array(group)
        else:
            group = np.array(group_value)

        month_result = []
        for i in range(group_num):
            s = time.time()
            month_X_val = X_val[group==i]
            month_y_val = y_val[group==i]
            val_loss_tem, metric_for_logging_tem, pred_time_tem, _ = _eval_estimator(
                config,
                estimator,
                X_train,
                y_train,
...
def evaluate_model_pseudo_CV(
    ...
    for train_index, val_index in kf:
        split_ind = int(0.75*len(val_index))
        val_index = np.array(val_index)
        tmp_train = list(val_index[:split_ind])
        true_train_index = true_train_index + tmp_train
        tmp_val = list(val_index[split_ind:])
        all_val_indexes.append(tmp_val)
    
    train_index = true_train_index
    if shuffle:
        train_index = rng.permutation(true_train_index)
    ...
    if weight is not None: 
        fit_kwargs["sample_weight"] = weight[train_index]
    ...
    for i in range(n):
        ...
        #     break
    # print("--------------------------------------------------")
    # print('val_loss_folds',val_loss_folds)
...
def evaluate_model_CV_V2(
    ...
    if "sample_weight" in fit_kwargs:
        ...
        weight = weight_val = None
    # print("--------------------------------------------------")
    # fixed_train_index, tmp_val_index = next(kf)

    tmp = np.array([i for i in range(len(X_train_split))])
    len_train = int(0.7*len(X_train_split)) # 0.7, 0.3-> 5 1, 2,3,4,5
    fixed_train_index, tmp_val_index = tmp[:len_train], tmp[len_train:]

    len_val_per_split = int(len(tmp_val_index) / n)
    start = 0
    all_val_indexes = []
    for j in range(n):
        if j != n-1:
            all_val_indexes.append(tmp_val_index[start : start + len_val_per_split])
        else:
            all_val_indexes.append(tmp_val_index[start:])
        start += len_val_per_split

    # print(all_val_indexes)

    new_kf = []
    for i in range(n):
        tmp_train = fixed_train_index
        for j in range(n):
            if j != i:
                tmp_train = np.concatenate((tmp_train, all_val_indexes[j]))
        new_kf.append((tmp_train, all_val_indexes[i]))


    for train_index, val_index in new_kf:
        if shuffle:
            train_index = rng.permutation(train_index)
        if isinstance(X_train_all, pd.DataFrame):
            X_train = X_train_split.iloc[train_index]
...
def evaluate_model_holdout_V2(
    ...
    # split train and validation
    len_train = int(0.75*len(X_train_split)) # first 75% as train
    train_index = np.array([i for i in range(len_train)])
    if shuffle:
        train_index = rng.permutation(train_index)
    if isinstance(X_train_all, pd.DataFrame):
        X_train = X_train_split.iloc[train_index]
    ...
    # get valid index all_val_indexes: [[split1], [split2], ...]
    tmp_val_index = np.array([i for i in range(len_train, len(X_train_split))])
    len_val_per_split = int((len(X_train_split) - len_train) / n)
    start = 0
    all_val_indexes = []
    for j in range(n):
        if j != n-1:
...
def compute_estimator(
    ...
    elif "pseudo_cv" == eval_method:
        val_loss, metric_for_logging, train_time, pred_time = evaluate_model_pseudo_CV(
            config_dic,
            estimator,
            X_train,
            y_train,
    ...
    elif "holdout_V2" == eval_method:
        val_loss, metric_for_logging, train_time, pred_time = evaluate_model_holdout_V2(
            config_dic,
            estimator,
            X_train,
            y_train,
    ...
    elif "cv_V2" == eval_method:
        val_loss, metric_for_logging, train_time, pred_time = evaluate_model_CV_V2(
            config_dic,
            estimator,
            X_train,
            y_train,
