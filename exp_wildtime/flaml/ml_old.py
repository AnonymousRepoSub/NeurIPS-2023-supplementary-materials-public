...
import logging
from flaml.group import get_test_group
...
def get_val_loss(
    ...
    # # print("---------------------month--------------------") # check
    tmp_dict = get_test_group()
    group_value = tmp_dict['group_value']
    group_num = tmp_dict['group_num']
    train_len = tmp_dict['train_len']

    # print(group_num)
    # print(group_value)

    # print(tmp_dict, len(X_train))
    if train_len == len(X_train) and group_num > 0:
        group_num = tmp_dict['group_num']
        length = len(X_val)
